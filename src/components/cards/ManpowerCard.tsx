    ...
    <Card className="flex flex-col p-[15px]">
      <CardTitle
        right={
          <Link
            to="/reports/manpower"
            className="text-[12.5px] font-medium leading-4 text-brand-link underline decoration-brand-link/60 underline-offset-2"
          >
            View Report
          </Link>
        }
      ...
      <div className="mt-[13px] grid grid-cols-[0.9fr_1.2fr_0.9fr] overflow-hidden rounded-[9px] border border-line">
        {manpower.stats.map((s, i) => (
          <div key={s.label} className={`px-[12px] py-[13px] ${i > 0 ? 'border-l border-line' : ''}`}>
            ...
            <p className={`mt-[6px] text-[17px] font-bold leading-6 ${STAT_TONES[s.tone] ?? 'text-ink'}`}>
              {s.value}
            </p>
          </div>
        ))}
        ...
        <div className="flex-1 space-y-[15px]">
          {manpower.attendance.map((a) => (
            <div key={a.key} className="flex items-center">
              ...
              <span className="ml-auto text-[13px] font-bold leading-4" style={{ color: a.color }}>
                {a.value}
              </span>
              <span className="ml-[14px] w-[46px] text-right text-[12px] font-semibold leading-4 text-ink-muted">
                {a.percent}
              </span>
            </div>
          ))}
