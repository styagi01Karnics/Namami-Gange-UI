...
const TONES = {
  ok: { bg: 'bg-ok-soft', chip: 'bg-ok', text: 'text-ok', icon: CctvIcon },
  danger: { bg: 'bg-danger-soft', chip: 'bg-danger', text: 'text-danger', icon: ico('mdi:cctv-off') },
  warn: { bg: 'bg-warn-soft', chip: 'bg-warn', text: 'text-warn', icon: ico('fluent:wrench-32-filled') },
}
...
function MiniStat({ item, className = '' }) {
  ...
  return (
    <div className={`rounded-[11px] px-[13px] py-[13px] text-center ${tone.bg} ${className}`}>
    ...
    <section className="rounded-[12px] border border-line bg-white p-[15px] shadow-card">
      <div className="flex items-center gap-[11px]">
        <span className="flex h-[42px] w-[42px] shrink-0 items-center justify-center rounded-[11px] bg-brand-soft">
          <CctvIcon size={22} className="text-brand" />
        </span>
      ...
      <MiniStat item={maintenance} className="mt-[13px]" />
    </section>
  )
}
