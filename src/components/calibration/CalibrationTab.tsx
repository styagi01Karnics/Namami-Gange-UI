...
/** Shared by the Remote Calibration page and its tab in STP Management. */
export default function CalibrationTab() {
  const [points, setPoints] = useState(() =>
    Object.fromEntries(calibrationPoints.map((p) => [p.key, p.defaultChecked])),
  )

  // S.No. numbers the visible rows, so it stays 1..n as the point filter changes
  const rows = useMemo(
    () => calibrationRows.filter((r) => points[r.point]).map((r, i) => ({ ...r, sno: i + 1 })),
    [points],
  )
