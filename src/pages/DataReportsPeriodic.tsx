...
export default function DataReportsPeriodic() {
  return (
    <AppLayout breadcrumbs={[{ label: 'Data Reports' }, { label: 'Monthly, Quarterly, Yearly' }]}>
      <PageToolbar dateLabel="May 2026" />
            ...
            <tbody>
              {rows.map((row, i) => (
                <tr key={i} className="border border-[#d8edff] bg-white">
                  <td className="px-[16px] py-[24px] text-[14px] font-[510] leading-[22px] whitespace-nowrap text-[#07121e]">
                    {row.ts}
                  </td>
                  {row.values.map((v, j) => (
